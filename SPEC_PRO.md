...
8. Works with: common MES exports, SAP exports, generic CSV (timestamp, equipment/reason, duration)
...
### About the data
- **Commercial MES exports are the most structured.** Clean column names, consistent date formats, proper event/OEE separation. The current parsers are battle-tested on these.
...
- **Generic CSVs are the universal fallback.** Any plant can export *something* with timestamps, equipment names, and durations. A flexible CSV parser with column auto-detection covers 70% of the market that doesn't use a commercial MES or SAP.
...
- **Floor-mounted OEE displays** — hardware OEE display. $5,000+ per machine. Shows real-time OEE on the floor. Doesn't analyze anything.
...
The current parsers are built for a specific MES export shape. Handling arbitrary CSV/Excel files with different column names, date formats, and structures is the biggest technical risk.
...
- Start with the 3 most common formats (common MES, generic CSV, Excel with headers)
- Add formats based on actual user uploads (log which formats fail)

**Acceptance criteria:** Upload a generic CSV with columns "Date, Machine, Duration_Min, Category" → get a valid analysis. Upload a commercial MES export → auto-detected, no user input needed.
